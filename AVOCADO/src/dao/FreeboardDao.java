...
public class FreeboardDao {
  ...
  public int deleteFreeboard(String no) {
    int result = 0;
    String query = " delete from TA_FreeBoard " + " where freeboard_no='" + no + "' ";
    try {
      connection = common.getConnection();
      ps = connection.prepareStatement(query);
      result = ps.executeUpdate();
  ...
  public int saveFreeboard(FreeboardDto dto) {
    int result = 0;
    String query = "insert into TA_FreeBoard \r\n" + "(freeboard_no,freeboard_title,freeboard_content,freeboard_regname,freeboard_regdate,freeboard_attach) \r\n" + "values \r\n" + "('" + dto.getFreeboard_no() + "','" + dto.getFreeboard_title() + "','" + dto.getFreeboard_content() + "','"
        + dto.getFreeboard_regName() + "',TO_DATE('" + dto.getFreeboard_regDate() + "','YYYY-MM-DD HH24:MI:SS'),'" + dto.getFreeboard_attach() + "')";
    try {
      connection = common.getConnection();
      ps = connection.prepareStatement(query);
      result = ps.executeUpdate();
