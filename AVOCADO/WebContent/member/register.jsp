			...
			<form name="register" onsubmit="return false">
				<div class="register-box animate__animated animate__fadeIn">
					...
					<div class="login-btn">
						...
						<button type="button" class="login-btn-login" onclick="goBack()">로그인
							페이지로</button>
					</div>
				</div>
			</form>
														...
														.ajax({
															...
															success : function(
																...
																$(".emailTip")
																		.html(
																				data);
																if (/^[A-Za-z0-9_.]+[A-Za-z0-9]*[@]{1}[A-Za-z0-9]+[A-Za-z0-9]*[.]{1}[A-Za-z]{1,3}$/
																		.test(email)) {
																	if ($
																			.trim(data) == "사용가능한 이메일 입니다.") {
																		$(
																				'.emailTip')
																				.css(
																						'display',
					...
					.keyup(
							function() {
								if (/^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[$@$!%*?&])[A-Za-z\d$@$!%*?&]{8,16}/
										.test(register.t_password.value)) {
									if ($('#input-pw').val() != $(
											'#input-pw-check').val()) {
										$('.passwordCheckTip').css('display',
												'block');
										$('.passwordCheckTip').css('color',
												'#f82a2aa3');
										$('.passwordCheckTip').html(
												'비밀번호 일치하지 않음');
										validPassword = false;
