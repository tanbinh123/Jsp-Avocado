...
public class QnaDao {
	 ...
	 DBOracleConnection common = new DBOracleConnection();
	  ...
	  ResultSet rs = null;	  
		...
		public int deleteQna(String no) {
			int result =0;
			String query=" delete from TA_qna " + 
						 " where qna_no='"+no+"' ";
			try {
				connection = common.getConnection();
				ps  = connection.prepareStatement(query);
		...
		public int saveQna(QnaDto dto) {
			...
			String query ="insert into TA_qna \r\n" + 
					...
					"values \r\n" + 
					"('"+dto.getqna_no()+"','"+dto.getqna_title()+"','"+dto.getqna_content()+"','"+dto.getqna_regName()+"',TO_DATE('"+dto.getqna_regDate()+"','YYYY-MM-DD HH24:MI:SS'),'"+dto.getqna_attach()+"')";
			try {
				connection = common.getConnection();
				ps  = connection.prepareStatement(query);
				result  = ps.executeUpdate();			
	...
	//목록조회
		public ArrayList<QnaDto> getQna(String select,String search){
			...
			String query =  " select b.qna_no, b.qna_title, a.member_name, to_char(b.qna_regdate,'yyyy-MM-dd'), b.qna_attach, b.qna_hit, c.qna_likestack\r\n " + 
							" from TA_qna b, \r\n " + 
							"        ta_member a, \r\n " + 
							"        (select like_postnumber, count(*) as qna_likestack   \r\n" + 
							"        from ta_qnalike \r\n " + 
			...
			try {
				...
				System.out.println("getqna() query 오류: "+query);
			}catch(Exception ee) {
				System.out.println("getqna() 오류");
			}finally {
				common.close(connection, ps, rs);
			}		
