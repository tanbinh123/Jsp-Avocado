...
//번호생성
  public String getKickboardNo() {
      ...
      try {
          ...
          System.out.println("getKickboardNo() query 오류: "+query);
      }catch(Exception ee) {
          System.out.println("getKickboardNo() 오류");
      }finally {
...
//등록
 public int saveKickboard(KickboardDto dto) {
     ...
     String query ="insert into TA_KICKBOARD \r\n" + 
             ...
             "values \r\n" + 
             "('"+dto.getKickboard_no()+"','"+dto.getKickboard_name()+"','"+dto.getKickboard_type()+"',"+dto.getKickboard_rentstats()+",'"+dto.getKickboard_location()+"',TO_DATE('"+dto.getKickboard_regdate()+"','YYYY-MM-DD HH24:MI:SS'),"+dto.getKickboard_totalusedtimes()+",'"+dto.getKickboard_img()+"',"+dto.getKickboard_battery()+")";
     try {
         connection = common.getConnection();
         ps  = connection.prepareStatement(query);
         result  = ps.executeUpdate();           
     }catch(SQLException se) {
         System.out.println("saveKickboard() query 오류: "+query);
     }catch(Exception ee) {
         System.out.println("saveKickboard() 오류");
     }finally {
         common.close(connection, ps, rs);
     }       
     return result;
...
//목록조회
 public ArrayList<KickboardDto> getKickboardValue(){
     ...
     try {
         ...
         System.out.println("getKickboardValue() query 오류: "+query);
     }catch(Exception ee) {
         System.out.println("getKickboardValue() 오류");
     }finally {
         common.close(connection, ps, rs);
     }       
 ...
 public ArrayList<KickboardDto> getKickboardRentStatsOK() {
   ...
   try {
     ...
     System.out.println("getKickboardRentStatsOK() query 오류: " + query);
   } catch (Exception ee) {
     System.out.println("getKickboardRentStatsOK() 오류");
   } finally {
     common.close(connection, ps, rs);
   }
