            ...
            <div class="adminPage-main-content-container">
                   ...
                   <form name="searchUser">
					...
					if (request.getParameter("t_kickboard_no") != null) {
					  no = request.getParameter("t_kickboard_no");
					  serchType = "kickboard";
					} else if (request.getParameter("t_member_no") != null){
					  no = request.getParameter("t_member_no");
					  serchType = "member";
					} else {
					  serchType = "";
					}
					...
					ArrayList<RentDto> rentArray = dao.getRentHistory(no, serchType);
						if (rentArray.size() == 0) {
						  out.print("검색 결과가 없습니다.");
								...
								<table>
								<tr>
								<th>렌트 번호</th><th>멤버 번호</th><th>멤버 이름</th><th>킥보드 번호</th><th>대여 시간</th><th>반납 시간</th><th>총 이용 시간(초)</th><th>대여 중 여부</th><th>요금</th>
								</tr>
								
							<% 
							DecimalFormat df = new DecimalFormat("###,###,###");
							for (int i = 0; i < rentArray.size(); i++) { 
							int usedDate = rentArray.get(i).getRent_useddate();
								String usedDateStr = (usedDate/60) + "분 " + (usedDate % 60) + "초";
								%>
							<tr>
								<td class= "td_center"><%=rentArray.get(i).getRent_no() %></td>
								<td class= "td_center" title="멤버 정보 조회" style="cursor:pointer;" onClick="location.href='/member/admin/serchMember.jsp?t_member_no=<%=rentArray.get(i).getRent_member_no()%>'"><%=rentArray.get(i).getRent_member_no() %></a></td>
								<td class= "td_center" title="멤버 정보 조회" style="cursor:pointer;" onClick="location.href='/member/admin/serchMember.jsp?t_member_no=<%=rentArray.get(i).getRent_member_no()%>'"><%=rentArray.get(i).getMember_member_name() %></a></td>
								<td class= "td_center" title="킥보드 정보 조회" style="cursor:pointer;" onClick="location.href='/member/admin/serchKickboard.jsp?t_kickboard_no=<%=rentArray.get(i).getRent_kickboard_no()%>'"><%=rentArray.get(i).getRent_kickboard_no() %></a></td>
								<td><%=rentArray.get(i).getRent_startdate() %></td>
								<td><%=rentArray.get(i).getRent_enddate() %></td>
								<td class= "td_center"><%=usedDateStr %></td>
								<td class= "td_center"><% if (rentArray.get(i).getRent_stats() == 0 ) {%>
								  	대여 중
								<% } else {%>
									반납 완료
								  <% } %>
								</td>
								<td class= "td_center"><%=df.format(rentArray.get(i).getRent_cost()) %>원</td>
							</tr>
						<% }%>
						</table>
							<%}%>
