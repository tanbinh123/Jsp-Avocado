...
public class NoticeDao {
  ...
  private String getQuery(String methodName) {
    if (equalsCheck(methodName, "getNoticeList"))
      return "select no, title, attach, content, reg_name, to_char(reg_date,'yyyy-MM-dd'), hit from h00_notice order by no desc";
  ...
  private String getQuery(String methodName, NoticeDto dto) {
    if (equalsCheck(methodName, "saveNotice"))
      return "insert into h00_notice (no,title,content,attach,reg_name,reg_date,hit) values ('" + dto.getNo() + "','" + dto.getTitle() + "','" + dto.getContent() + "','" + dto.getAttach() + "','" + dto.getReg_name() + "','" + dto.getReg_date() + "','" + dto.getHit() + "')";
    if (equalsCheck(methodName, "updateNotice"))
      return "update h00_notice set title='" + dto.getTitle() + "', content='" + dto.getContent() + "', attach ='" + dto.getAttach() + "', reg_name ='" + dto.getReg_name() + "', reg_date ='" + dto.getReg_date() + "' where no ='" + dto.getNo() + "'";
  ...
  private String getQuery(String methodName, String no) {
    if (equalsCheck(methodName, "deleteNotice"))
      return " delete from h00_notice where no ='" + no + "'";
